...
namespace fullsail_ai {
	namespace algorithms {
		...
		class PathSearch
		{
			std::vector<SearchNode*> m_Tiles;
			std::unordered_map<Tile*, SearchNode*> SearchGraph;

			std::queue<PlannerNode*> open;
			std::unordered_map<Tile*, PlannerNode*> m_visted;
			...
			std::priority_queue<PlannerNode*, std::vector<PlannerNode*>, Compare>openENHANCED;

			float PathSearch::estimate(Tile* start, Tile* goal);
			PriorityQueue<PlannerNode*> openEnhancer;
